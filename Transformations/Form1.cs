...
namespace Transformations
{
    public partial class Form1 : Form
    {
        ...
        public void Render()
        {
            ...
            //1st set of 9
            TransformAndDraw(Matrix.RotationZ(angle) * Matrix.Translation(5, 10, 5));
            TransformAndDraw(Matrix.RotationZ(angle) * Matrix.Translation(0, 10, 5));
            TransformAndDraw(Matrix.RotationZ(angle) * Matrix.Translation(-5, 10, 5));

            TransformAndDraw(Matrix.RotationYawPitchRoll(angle, angle / 2, angle / 3) * Matrix.Translation(5, 0, 5) * Matrix.RotationY(angle));
            TransformAndDraw(Matrix.RotationYawPitchRoll(angle, angle / 2, angle / 3) * Matrix.Translation(0, 0, 5) * Matrix.RotationY(angle));
            TransformAndDraw(Matrix.RotationYawPitchRoll(angle, angle / 2, angle / 3) * Matrix.Translation(-5, 0, 5) * Matrix.RotationY(angle));

            TransformAndDraw(Matrix.Scaling(0.5f, 2.0f, 0.5f) * Matrix.Translation(5, -10, 5));
            TransformAndDraw(Matrix.Scaling(0.5f, 2.0f, 0.5f) * Matrix.Translation(0, -10, 5));
            TransformAndDraw(Matrix.Scaling(0.5f, 2.0f, 0.5f) * Matrix.Translation(-5, -10, 5));


            //2nd set of 9
            TransformAndDraw(Matrix.RotationY(angle) * Matrix.Translation(5, 10, 0));
            TransformAndDraw(Matrix.RotationY(angle) * Matrix.Translation(0, 10, 0));
            TransformAndDraw(Matrix.RotationY(angle) * Matrix.Translation(-5, 10, 0));

            TransformAndDraw(Matrix.RotationYawPitchRoll(angle, angle / 2, angle / 3) * Matrix.Translation(5, 0, 0) * Matrix.RotationY(angle));
            //TransformAndDraw(Matrix.RotationYawPitchRoll(angle, angle / 2, angle / 3)  Matrix.Translation(0, 0, 0));
            TransformAndDraw(Matrix.RotationYawPitchRoll(angle, angle / 2, angle / 3) * Matrix.Translation(-5, 0, 0) * Matrix.RotationY(angle));

            TransformAndDraw(Matrix.Scaling(0.5f, 2.0f, 0.5f) * Matrix.Translation(5, -10, 0));
            TransformAndDraw(Matrix.Scaling(0.5f, 2.0f, 0.5f) * Matrix.Translation(0, -10, 0));
            TransformAndDraw(Matrix.Scaling(0.5f, 2.0f, 0.5f) * Matrix.Translation(-5, -10, 0));

            //3rd set of 9
            TransformAndDraw(Matrix.RotationX(angle) * Matrix.Translation(5, 5, -5));
            TransformAndDraw(Matrix.RotationX(angle) * Matrix.Translation(0, 5, -5));
            TransformAndDraw(Matrix.RotationX(angle) * Matrix.Translation(-5, 5, -5));
            ...
            TransformAndDraw(Matrix.Scaling(0.5f, 2.0f, 0.5f) * Matrix.Translation(5, -5, -5));
            TransformAndDraw(Matrix.Scaling(0.5f, 2.0f, 0.5f) * Matrix.Translation(0, -5, -5));
            TransformAndDraw(Matrix.Scaling(0.5f, 2.0f, 0.5f) * Matrix.Translation(-5, -5, -5));
        ...
        private void TransformAndDraw(Matrix transform)
        {
        ...
        private void SetupCamera()
        {
            ...
            device.Transform.View = Matrix.LookAtLH(
              new Vector3(0.0f, 10.0f, -35.0f),        //camera position
